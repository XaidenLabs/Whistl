...
function MatchRow({ fixture }: { fixture: TxFixture }) {
  const phase = matchPhase(fixture.StartTime);
  const label = kickoffLabel(fixture.StartTime);

  return (
    <Link
      href={`/pulse/match/${fixture.FixtureId}`}
      className="block rounded-xl border border-line bg-ink-2 p-3.5 transition-colors hover:border-line/80 hover:bg-ink-3"
    >
      <div className="mb-2 flex items-center justify-between">
        <span className="truncate font-mono text-[9px] uppercase tracking-wider text-text-dim">
          {fixture.Competition}
        </span>
        <PhasePill phase={phase} label={label} />
      </div>
      <div className="flex items-center justify-between gap-3">
        <p className="min-w-0 truncate text-sm font-semibold text-text">
          {fixture.Participant1} <span className="text-text-dim">v</span> {fixture.Participant2}
        </p>
        <ChevronRight className="size-4 shrink-0 text-text-dim" aria-hidden />
      </div>
      <p className="mt-2 flex items-center gap-1.5 font-mono text-[10px] text-signal">
