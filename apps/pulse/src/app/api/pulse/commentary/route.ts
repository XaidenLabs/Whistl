...
export async function POST(req: Request) {
  ...
  try {
    ...
    if (card.source === "ace") {
      // Don't await the save! We want to return the UI response immediately so it shows "saving...".
      import("@/lib/whistl/server").then(({ saveCommentaryOnChain }) => {
        saveCommentaryOnChain({
          fixtureId,
