...
function FixtureCard({ f }: { f: TxFixture }) {
  ...
  // Ended: kicked off more than 2h ago
  const isEndedFallback = Date.now() > f.StartTime + 2 * 3600 * 1000;

  const { data: oddsData } = useSWR<OddsOk>(
    isEndedFallback ? null : `/api/txline/odds/${f.FixtureId}`,
    oddsFetcher,
  ...
  const isEnded = isEndedFallback && !isLive;
...
export default function MatchesPage() {
  ...
  const visibleLive = showAllLive ? liveOrUpcoming : liveOrUpcoming.slice(0, 12);
  const visibleEnded = showAllEnded ? ended : ended.slice(0, 12);
    ...
    <main className="mx-auto w-full max-w-6xl flex-1 px-4 sm:px-6 py-10">
      ...
      {!isLoading && !error && liveOrUpcoming.length > 0 && (
        <section>
          ...
          {liveOrUpcoming.length > 12 && !showAllLive && (
            <button
              type="button"
              onClick={() => setShowAllLive(true)}
              className="mt-6 w-full rounded-xl border border-line bg-ink-2 py-3 text-sm text-text-dim hover:bg-ink-3 transition-colors"
            >
              Show {liveOrUpcoming.length - 12} more upcoming matches
            </button>
          )}
        </section>
      )}
      ...
      {!isLoading && !error && ended.length > 0 && (
        <section className={liveOrUpcoming.length > 0 ? "mt-10" : ""}>
          ...
          {ended.length > 12 && !showAllEnded && (
            <button
              type="button"
              onClick={() => setShowAllEnded(true)}
              className="mt-6 w-full rounded-xl border border-line bg-ink-2 py-3 text-sm text-text-dim hover:bg-ink-3 transition-colors"
            >
              Show {ended.length - 12} more ended matches
            </button>
          )}
        </section>
      )}
